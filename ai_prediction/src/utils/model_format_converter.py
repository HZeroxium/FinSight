...
class ModelFormatConverter:
                ...
                try:
                    ...
                    if example_input is not None:
                        ...
                        if isinstance(example_input, torch.Tensor):
                            ...
                            example_input = [x.cpu() if isinstance(x, torch.Tensor) else x for x in example_input]
                        ...
                        scripted_model.save(target_dir / "model_torchscript.pt")  # Standard TorchScript name
                        scripted_model.save(target_dir / "scripted_model.pt")     # Alternative name
                        scripted_model.save(target_dir / "model.pt")             # Adapter expected name
            ...
            if state_dict_path.exists():
                ...
                for expected_name in expected_names:
                    ...
                    if not expected_path.exists():
                        ...
                        self.logger.debug(f"Created compatibility file: {expected_path}")
                        
        except Exception as e:
            self.logger.warning(f"Failed to create TorchScript compatibility files: {e}")
    ...
    def _create_example_input(self, model: ITimeSeriesModel) -> Optional[torch.Tensor]:
        ...
        try:
            ...
            if hasattr(model, 'feature_engineering') and model.feature_engineering is not None:
                try:
                    ...
                    self.logger.info(f"Using feature engineering output: {num_features} features")
                except Exception as fe_error:
                    self.logger.warning(f"Could not get feature names from feature engineering: {fe_error}")
                    # Fallback to basic feature columns
                    num_features = len(getattr(model, 'feature_columns', ['close']))
            else:
                # Fallback to basic feature columns
                num_features = len(getattr(model, 'feature_columns', ['close']))
                self.logger.warning(f"No feature engineering found, using basic features: {num_features}")
    ...
    def _create_triton_structure(
        ...
        try:
            ...
            for file_path in target_dir.iterdir():
                if file_path.is_file() and file_path.suffix in ['.pt', '.pkl', '.json', '.bin']:
