...
import sys
import os
from transformers import InformerConfig, InformerForPrediction, Trainer
...
class InformerTimeSeriesTrainer(Trainer):
    """
    Custom trainer for Informer model.
    """
    ...
    def compute_loss(
        ...
        """
        Custom loss computation for Informer model.
        """
        ...
        try:
            # Informer expects specific inputs during training
            outputs = model(
            ...
            logger.error(f"Informer forward failed: {e}")
            # Don't try fallback - raise the error immediately
            raise RuntimeError(f"Informer model forward pass failed: {e}") from e
...
class InformerStrategy(ModelStrategy):
    """
    Strategy implementation for Informer model.
    """
    ...
    def get_default_config(
        ...
        """Get default configuration for Informer."""
        # Use a reasonable lags_sequence - not the full context_length as that causes tensor issues
        config = {
    ...
    def get_default_peft_config(self) -> PEFTConfig:
        """Get default PEFT configuration for Informer."""
        return PEFTConfig(
            peft_method="lora",
            lora_r=8,
            lora_alpha=16,
    ...
    def create_model(self, **config_kwargs) -> InformerForPrediction:
        """Create Informer model."""
        config = InformerConfig(**config_kwargs)
        model = InformerForPrediction(config)
        self.logger.info(f"Created Informer model with config: {config_kwargs}")
        return model

    def prepare_data_collator(self) -> callable:
        """Return data collator for Informer."""
    ...
    def create_trainer_class(self) -> type:
        """Return Informer-specific trainer class."""
        return InformerTimeSeriesTrainer
