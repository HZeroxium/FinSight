"""
Standalone Informer experiment script for debugging.
...
class InformerTrainer(Trainer):
    """Custom trainer for Informer to handle specific loss computation."""

    def compute_loss(
        self, model, inputs, return_outputs=False, num_items_in_batch=None
    ):
        """
        Compute loss for Informer.
        """
...
class InformerExperiment:
    """Complete Informer experiment implementation."""

    def __init__(self, data_path: str, output_dir: str = "informer_experiment"):
        self.logger = LoggerFactory.get_logger("InformerExperiment")
        ...
        self.logger.info(
            f"Initialized Informer experiment with context_length={self.context_length}"
        )
    ...
    def create_sequences(self, data: np.ndarray) -> tuple[np.ndarray, np.ndarray]:
        ...
        for i in range(len(data) - self.context_length - self.prediction_length + 1):
            # Input sequence (only close price for Informer)
    ...
    def create_model(self) -> InformerForPrediction:
        """Create and configure the Informer model."""
        config = InformerConfig(
            context_length=self.context_length,
            prediction_length=self.prediction_length,
            num_time_features=0,  # No additional time features
            lags_sequence=[1, 2, 3, 7, 14, 21],  # Required for Informer
        ...
        self.logger.info(
            f"Created Informer model with {sum(p.numel() for p in model.parameters())} parameters"
        )
        return model

    def collate_fn(self, batch):
        """Custom data collator for Informer."""
        ...
        # Create required inputs for Informer
        past_time_features = torch.zeros(
    ...
    def train_model(
        self, model: InformerForPrediction, train_dataset: Dataset, val_dataset: Dataset
    ):
        """Train the model."""
    ...
    def evaluate_model(
        self, model: InformerForPrediction, test_dataset: Dataset
    ) -> dict:
    ...
    def run_experiment(self):
        ...
        try:
            self.logger.info("Starting Informer experiment")
...
def main():
    """Run the Informer experiment."""
    data_path = r"d:\Projects\Desktop\FinSight\finetuning\data\BTCUSDT_1d.csv"
    output_dir = (
        r"d:\Projects\Desktop\FinSight\finetuning\src\experiments\outputs\informer"
    )

    experiment = InformerExperiment(data_path, output_dir)
