"""
Standalone Autoformer experiment script for debugging.
...
class AutoformerTrainer(Trainer):
    """Custom trainer for Autoformer to handle specific loss computation."""

    def compute_loss(
        self, model, inputs, return_outputs=False, num_items_in_batch=None
    ):
        """
        Compute loss for Autoformer.
        """
...
class AutoformerExperiment:
    """Complete Autoformer experiment implementation."""

    def __init__(self, data_path: str, output_dir: str = "autoformer_experiment"):
        self.logger = LoggerFactory.get_logger("AutoformerExperiment")
        ...
        self.logger.info(
            f"Initialized Autoformer experiment with context_length={self.context_length}"
        )
    ...
    def prepare_data(self, df: pd.DataFrame) -> tuple[Dataset, Dataset, Dataset]:
        ...
        # For Autoformer, we need to use only close price as input since num_input_channels=1
        X_close_only = X[:, :, 3:4]  # Take only close price (index 3)

        # Scale targets separately
        y_scaled = self.target_scaler.fit_transform(y.reshape(-1, 1)).reshape(y.shape)

        # Split data
        n_samples = len(X_close_only)
        train_end = int(n_samples * self.train_ratio)
        val_end = int(n_samples * (self.train_ratio + self.val_ratio))

        X_train, y_train = X_close_only[:train_end], y_scaled[:train_end]
        X_val, y_val = X_close_only[train_end:val_end], y_scaled[train_end:val_end]
        X_test, y_test = X_close_only[val_end:], y_scaled[val_end:]
    ...
    def create_model(self) -> AutoformerForPrediction:
        """Create and configure the Autoformer model."""
        config = AutoformerConfig(
            context_length=self.context_length,
            prediction_length=self.prediction_length,
            num_time_features=0,  # No additional time features
            lags_sequence=[1, 2, 3, 7, 14, 21],  # Required for Autoformer
        ...
        self.logger.info(
            f"Created Autoformer model with {sum(p.numel() for p in model.parameters())} parameters"
        )
        return model

    def collate_fn(self, batch):
        """Custom data collator for Autoformer."""
    ...
    def evaluate_model(
        self, model: AutoformerForPrediction, test_dataset: Dataset
    ) -> dict:
    ...
    def run_experiment(self):
        ...
        try:
            self.logger.info("Starting Autoformer experiment")
...
def main():
    """Run the Autoformer experiment."""
    data_path = r"d:\Projects\Desktop\FinSight\finetuning\data\BTCUSDT_1d.csv"
    output_dir = (
        r"d:\Projects\Desktop\FinSight\finetuning\src\experiments\outputs\autoformer"
    )

    experiment = AutoformerExperiment(data_path, output_dir)
